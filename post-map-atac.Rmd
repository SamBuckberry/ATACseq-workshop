...
We can then run that script in parallel to process all the BAM files
```{bash}
cd aligned data

parallel sh ../process-atac-bam.sh {} ../reference/hg38.blacklist.bed ::: *_filt_fastp.bam
...
bam_list <- list.files("aligned_data/",
                       pattern = "_filt_fastp.bam$",
                       full.names = TRUE)

ids <- basename(bam_list) %>% str_remove_all("_filt_fastp_dedup.bam") %>%
...
Call peaks for each sample using macs3 
```{r}
...
lapply(X = 1:length(atac_bed), FUN = call_peaks_from_bed)
